...
public class Bulk {
	 ...
	 */
	public Bulk(String baseURL, String username, String password,
			String workflow, String rootMarker, String recordMarker,
			String idMarker, boolean formatAsXml) throws MalformedURLException {
		if (!baseURL.endsWith("/")) {
			baseURL += "/";
		}

		String address = baseURL + "Bulk.svc" + "/" + workflow + "/"
				+ recordMarker + "/" + idMarker;
	...
	/**
	 * Main method. Ex arguments: -h
	 * https://fingerprintengine.ptgels.com/TACO7500 -u user -p password -f txt
	 * -o out -x Send a directory content to some TACO client.
	 * 
	 * @throws Exception
	 */
	public static void main(String[] args) throws Exception {
		...
		options.addOption("x", "xml", false,
				"format each document as xml [plain text by default]");
		options.addOption("d", "documentmarker", true,
		...
		Bulk bulk = new Bulk(cl.getOptionValue("h", null), cl.getOptionValue(
				"u", null), cl.getOptionValue("p", null), cl.getOptionValue(
				"w", "MeSHXml"), cl.getOptionValue("r", "Documents"),
				cl.getOptionValue("d", "Document"),
				cl.getOptionValue("i", "ID"), cl.hasOption("x"));
	 ...
	 * request. data - The data you want to send
	 * 
	 * @param bulkRequest
	...
	public String postData(String bulkRequest) throws IOException {
		...
		try {
			...
			throw new IOException("Connection error (is server running at "
					+ endpoint + " ?): " + e);
		} finally {
			if (urlc != null)
				urlc.disconnect();
	...
	private void storeBulkResponse(Document document, String outputFolder)
		...
		for (Element node : document.getDescendants(new ElementFilter(
				recordMarker))) {
	...
	public String directoryToBulk(String directory) throws IOException {
		...
		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile()) {
				...
				// get the id and text
				String id = FilenameUtils.removeExtension(listOfFiles[i]
						.getName());
		...
		doc.setContent(root);
		XMLOutputter outputter = new XMLOutputter();
		String result = outputter.outputString(doc).replace(
